...
**Date:** 2026-08-18 (night, ~20:07 local)  
**Verdict:** Volume is huge. Decision quality is still not. Publication readiness is **NOT_PUBLISHABLE** (legal identity empty — do not invent it). Internal health **82/100** only means the issue ledger is mostly clear. It does **not** mean pages are distinctive.

Your instinct is right. The catalogue now has a full in-category comparison mesh (4,041 published pairs), 277 alternatives pages, CORE guides on all 11 maps, and **0** products missing major official media. Most of that is structurally complete template output: comparison outcomes with “comparable support” language, product-guide packs, and guide heroes that miss the teaching-visual bar. Dimensional quality scores look high because expected sections exist.
...
| Question | Answer |
| --- | --- |
| Are all pages complete? | **No.** Registry **5,747** published / **5,744** indexable. Legal identity empty. 7 CRM-map roadmap tools still missing. Capability-hub quality CLI still crashes. Fastmail and SaneBox have no two-substitute alternatives page. |
| Is content detailed, not missing? | **Mostly template-complete, not distinctive.** 4,041/4,041 comparisons pass the schema quality gate. **506** pairs are ≥50% “comparable support” boilerplate. **1,256 / 1,354** scored guides are CQ-P2. |
| Images / screenshots / videos present? | **Official media: 279 products, 510 active videos, 0 missing major coverage.** DirectAdmin has no vendor YouTube tour (honest); coverage is a first-party control-panel still. Guide library fails the teaching-visual size bar. **205** live-wired guide heroes 404 (`*-cover-v3.png`). No `public/compare/` art. |
| Generated visuals follow the Cursor rule? | **Hubs yes. Guides no.** Rule: 1536×1024, typically ~1 MB+, unique, no shared placeholders, no `?v=`. Guide PNGs: **6,625**, median **97 KB**, 1,329 ≥900 KB. Live-wired heroes that exist: median **110 KB**; 487/1,141 ≥900 KB. Unique srcs; no `?v=`. |
| Comparisons detailed? | **Structurally filled, not editorially distinctive.** All 4,041 are `approved` + `research complete` + indexable. 7,183 outcomes use “comparable support”; 3,386 are `low` confidence. A 120-page quality sample scores **88** (all CQ-P3) because modules exist. |
| Comparisons for every in-category permutation? | **Yes, for every primary category.** 3,964 / 3,964 expected pairs (100%). Cross-category pairs are out of that mesh. |
| All guides that should exist, and detailed? | **CORE maps: yes** (`content:clusters:gaps` = 0 × 11). **1,224 / 1,346** guides are product-tied. Nine CQ-P1 pages (customer-service worth-it / what-is at 60–65). |
...
| Links between pages proper? | **Graph yes:** 32,253 edges, 0 orphans, 0 weak. Alternatives hub only lists **6** indexable children (271 noindex shells). Wiring ≠ unique editorial journeys. |
...
| Content quality | `content-audit-cli` `--no-write` | reviews / guides / comparisons (n=120) / best / industry / use-case / resource / feature / requirement / crm |
...
| Comparison mesh | live `getComparisons` × primary-category pairs | **100%** every primary category |
...
| Category maturity | `assessCategoryMaturity` | CRM **MATURE**; other primary cats **TOOL_READY** |

`content-audit capability` still crashes (hub `priorities` are strings; `scenarios[0].bestWhen` missing). Capability hubs were not quality-scored.
...
| Official videos | 510 · **0 missing major** (DirectAdmin = screenshot only) |

Same-day earlier snapshots: morning still had a thin non-CRM comparison mesh and 12 missing official videos. Those existence gaps closed. Distinctiveness did not.
...
4. **Comparison distinctiveness** — mesh existence is done. 506 pairs are majority “comparable support”; 3,386 low-confidence outcomes. Do not invent verdicts.
...
DirectAdmin vendor YouTube stays an **honest gap**. Do not add reseller clips. Major coverage is the first-party panel still.
...
277 catalogue pages; **6 indexable** (researched + approved). 270 are non-indexable shells (existing slugs only, no invented reasons). Hub `/alternatives/` graph children: **6**. Fastmail and SaneBox remain without a page — only one honest substitute each.

### CRM master map

SEO content-coverage agent: 207 rows, **0 thin indexable findings**, 7 roadmap gaps tracked (not raised as production P1).
...
DirectAdmin: **0 official videos**. JBMC has no vendor YouTube tour that passes source verification; reseller clips do not count. Coverage is `public/software/directadmin/vendor-ui-panel.png` (first-party control-panel still from directadmin.com).
...
Every published guide has a `heroVisual` field. **205** of those srcs are missing files (`*-cover-v3.png`). Live-wired files that exist: median **110 KB**; **487 ≥900 KB** / **654** under 900 KB. Unique srcs; no `?v=` query strings.

Media SEO: 624 `<img>` on 19 pages; 29 YouTube posters oEmbed-live.
...
The graph is wiring. It does not make every product-guide pack a unique journey.
...
| Field CWV | collector wired (consent-gated); **not CrUX**; local TTFB 12/19 over warn, findings suppressed |
...
2. Guide PNGs at the teaching-visual bar **and** the 205 missing `-cover-v3.png` paths resolved.
3. Comparison pages a buyer can use — not 4,041 schema-valid permutations with boilerplate outcomes. Prefer researched, distinctive pages for high-intent pairs; do not invent verdicts.
...
Affiliate programmes on 251 products remain a commercial backlog. DirectAdmin without a vendor YouTube tour remains honest.
...
| `npx tsx scripts/content-audit-cli.ts all` | not re-run | Same hub loader |
| `npm run catalogue:coverage` | still broken as of earlier today | Processing-record `maturityTier` enum mismatch |

Comparisons quality CLI was sampled at **120** pages. Structural stats above cover the full 4,041.
...
| Artifact | Path |
| --- | --- |
| This report | `docs/reports/site-wide-editorial-audit-2026-08-18-night.md` |
| Earlier same-day reports | `docs/reports/site-wide-editorial-audit-2026-08-18.md`, `…-evening.md` |
