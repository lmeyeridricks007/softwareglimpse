...
/** Worked-example marketing team used across every EM product guide. */
const TEAM = "a 6-person marketing team";
const TEAM_SEATS = 6;
...
function featurePhrase(ctx: ProductGuideContext): string {
  if (ctx.supportedFeatureLabels.length === 0) {
    return "core email-marketing jobs (campaigns, lists, automations)";
  }
...
function coreLoopPhrase(ctx: ProductGuideContext): string {
  if (ctx.coreLoopLabels.length === 0) {
    return "lists, campaigns, segments, and automations";
  }
...
function bestForPhrase(ctx: ProductGuideContext): string {
  if (ctx.bestFor.length === 0) {
    return "marketing teams that will own list quality, sends/contacts, and ESP ↔ CRM sync weekly";
  }
...
function positioningSentence(ctx: ProductGuideContext): string {
  ...
  return `${ctx.productName} is evaluated here as email marketing / ESP tooling — not a CRM system of record.`;
}
...
function planSoftener(ctx: ProductGuideContext): string {
  ...
  return `${ctx.productName} is often sold on seats, sends/contacts, or contact-sales packaging in our snapshot — treat homepage tiles as marketing, not a bill of materials. Confirm live packaging on the pricing page.`;
}

function pricingPointer(ctx: ProductGuideContext): string {
  return `Never invent list prices here — confirm seats, sends/contacts, and quote terms on ${ctx.pricingHref}.`;
}
...
function researchCallout(
  ...
  const framing: Record<CrmProductGuideKind, string> = {
    ...
    plans: "Weigh these when you pick seats, sends/contacts, and a qualifying tier.",
...
function relatedLinks(
  ...
  const siblings = (
    [
      ...
      ["plans", "Plans / seats vs sends/contacts"],
      ["worth-it", "Worth it?"],
    ] as const
  )
    .filter(([k]) => k !== kind)
  ...
  return {
    ...
    links: [
      ...
      },
      {
        ...
        description: "Researched plans, sends/contacts, and sources.",
      },
      ...siblings,
      {
...
function interactiveCta(
  ...
  if (kind === "plans") {
    return {
      ...
      body: `Email marketing often mixes seats, sends/contacts, and quote terms. Use the researched pricing page — do not invent totals in a spreadsheet.`,
      href: ctx.pricingHref,
  ...
  if (kind === "worth-it") {
    return {
      ...
      body: `If ${ctx.productName} is close but not obvious, read how to choose email marketing and compare finalists with the same requirements — no affiliate-ordered rankings.`,
      href: "/guides/how-to-choose-sales-intelligence/",
...
function buildEmSetupBlocks(ctx: ProductGuideContext): GuideBlockInput[] {
  const name = ctx.productName;
  const campaigns = ctx.feature("email-campaigns");
  const segmentation = ctx.feature("segmentation");
  const automations = ctx.feature("automation-workflows");
  const newsletters = ctx.feature("newsletter-builder");
  const startPlan =
    ctx.freePlanNames[0] ??
    ctx.entryPlanName ??
  ...
  return compact([
    {
      ...
      id: "quick-answer",
      title: "Quick answer",
      body: `Set up ${name} in this order: qualify contacts/sends for day-one outreach, name one campaign owner, build one subscriber segment, connect ESP ↔ CRM sync, enable campaigns or automations, then have a non-admin import contacts, segment, send a campaign, and read a report.${quickGateHint(ctx)} You’re done when that walkthrough works — not when every optional add-on is switched on.`,
      bullets: [
        `Start on ${startPlan}`,
    ...
    mustNiceMatrix(ctx, "setup", [
      ...
      {
        feature: "ESP ↔ CRM sync for logged activity",
        mustHave: true,
        niceToHave: false,
    ...
    researchCallout(ctx, "setup"),
    {
      ...
      body: `Write five day-one jobs, map each to researched ${name} packaging, and pick the cheapest tier that covers all five — including sends/contacts if usage-based.\n\n1. List the five things the marketing team must do on day one.\n2. Match each one to researched ${name} packaging. ${gatedHintSentence(ctx)}\n3. Pick the cheapest package that covers all five.\n4. Check contacts and send limits, not just feature checkmarks. ${capacity ?? `Confirm contact/send caps for ${name} before inviting everyone.`}\n\n${trialSentence(ctx)}\n\n${pricingPointer(ctx)}\n\nWorked example: ${TEAM} needs ${coreLoopPhrase(ctx)} plus ESP ↔ CRM sync on day one in ${name}. ${blocker ? `${blocker} They set up on ${ctx.paidEntryPlanName ?? ctx.entryPlanName ?? "a paid package"} rather than a capped tier.` : `They start on ${startPlan} and note which capabilities would force an upgrade.`}`,
      tip: `Do not configure against a demo tenant. Demos frequently run with inflated sends/contacts or ${ctx.topPlanName ?? "top-tier"} access — ask which package you are looking at.`,
      ...
      items: [
        {
          ...
          answer: `A focused pod can finish day-zero setup in one working day if contacts/sends, subscriber segment, ESP ↔ CRM sync, and one outreach channel are already decided. Multi-week “setup” usually means undecided packaging or missing ownership.`,
        },
        {
...
function buildEmImplementationBlocks(
  ctx: ProductGuideContext,
): GuideBlockInput[] {
  const name = ctx.productName;
  const automations = ctx.feature("automation-workflows");
  ...
  return compact([
        ...
        },
        {
          ...
          body:
            ctx.weaknesses.length > 0
              ...
              : `Confirm credit discipline and ESP ↔ CRM sync before you promise a rollout date for ${name}.`,
        },
      ]) as Array<{ label: string; body?: string }>,
    },
    {
    ...
    },
    {
      ...
      items: [
        {
          question: `What if sends/contacts run out before day 30?`,
...
function buildEmMigrationBlocks(ctx: ProductGuideContext): GuideBlockInput[] {
  const name = ctx.productName;
  ...
  return compact([
    {
      ...
      id: "quick-answer",
      title: "Quick answer",
      body: `Migrate into ${name} with a list inventory, field map, pilot import, dual-run week, and ESP ↔ CRM sync validation — then cut over only when the marketing team trusts the contacts.${quickGateHint(ctx)} Prove one segment before you move the whole book of lists and automations.`,
      bullets: [
      ...
      items: [
        {
          ...
          body: `${positioningSentence(ctx)} A migration that moves rows but breaks suppressions or owners will burn sends/contacts and trust.`,
        },
        {
    ...
    },
    {
      ...
      caption: `Prove a small ${name} import before you move the whole email marketing book.`,
    },
    phaseChecklist(ctx, "migration", [
      ...
      },
      {
        ...
        description: "Spot-check contacts and ESP ↔ CRM sync before cutover.",
      },
    ]),
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          answer: `Owner remaps, phone/email field meanings, and ESP ↔ CRM sync direction. Pilot those three before bulk.`,
        },
        {
          question: `How do sends/contacts factor into migration?`,
...
function buildEmPlansBlocks(ctx: ProductGuideContext): GuideBlockInput[] {
  const name = ctx.productName;
  const capacity = capacitySentence(ctx);
  const blocker = seatCapBlocker(ctx);
  ...
  return compact([
    ...
    mustNiceMatrix(ctx, "plans", [
      {
        feature: "Campaigns + ESP ↔ CRM sync",
        mustHave: true,
        niceToHave: false,
        notes: "Day-one floor",
      },
    ...
    researchCallout(ctx, "plans"),
    {
      ...
      body: `1. Write five day-one jobs for the marketing team.\n2. Mark which require seats vs sends/contacts vs add-on packs.\n3. Circle the highest gated must-have — that sets the floor tier.\n4. ${gatedHintSentence(ctx)}\n\nWorked example: ${TEAM} needs campaigns, ESP ↔ CRM sync, and automations on day one in ${name}. Extra segmentation and AI are nice later, so they refuse to let those upsells set the package.`,
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          answer: `Common for email marketing. Compare with a written must-have sheet and quote diligence — not blog “starting at” figures.`,
        },
        {
          question: `Seats or sends/contacts — which matters more?`,
          answer: `Whichever your day-one jobs consume. Many teams under-buy sends/contacts and over-buy spectator seats — reverse that.`,
        },
      ],
    },
...
function buildEmWorthItBlocks(ctx: ProductGuideContext): GuideBlockInput[] {
  const name = ctx.productName;
  ...
  return compact([
    ...
    researchCallout(ctx, "worth-it"),
    {
      ...
      figure: teachingFigure(
        ...
        "worth-it",
        1,
        `Fit ${name} to your email marketing motion before you talk ROI.`,
      ),
    },
    {
      type: "step",
    ...
    },
    {
      ...
      items: [
        ...
        {
          question: `What if sends/contacts look cheap but seats are expensive?`,
          answer: `Model the constraint you will hit first. Many teams exhaust sends/contacts before seats — confirm both on the pricing page.`,
        },
        {
...
export function buildEmBlocksForKind(
  ctx: ProductGuideContext,
  kind: CrmProductGuideKind,
): GuideBlockInput[] {
...
function emBlocksForKind(
  ...
  switch (kind) {
    case "implementation":
      return buildEmImplementationBlocks(ctx);
    case "migration":
      return buildEmMigrationBlocks(ctx);
    case "setup":
      return buildEmSetupBlocks(ctx);
    case "plans":
      return buildEmPlansBlocks(ctx);
    case "worth-it":
      return buildEmWorthItBlocks(ctx);
    default: {
