...
/** Matches CRM Finder / Cost Calculator workspace panels on tint backgrounds. */
const PANEL =
...
export function CrmVendorScorecardApp({
  ...
  const syncShortlistToProfile = useCallback(
    (productIds: string[]) => {
      const current = loadCrmDecisionProfile();
      if (!current) return;
      const next = touchCrmDecisionProfile(current, {
  ...
  function useProfile() {
    const loaded = loadCrmDecisionProfile();
    if (!loaded) return;
    setProfile(loaded);
    const criteria = generateCriteriaFromProfile(loaded);
    const productIds =
  ...
  function downloadCsv() {
    ...
    a.href = url;
    a.download = "crm-vendor-scorecard.csv";
    a.click();
        ...
        <div className="min-w-0 space-y-8">
            ...
            <div className="flex flex-wrap items-end justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-[var(--sg-color-text-muted)]">
                  Select 2–5 CRM products. Order is not a ranking.
                </p>
              </div>
              <Button
                type="button"
          ...
          <section aria-labelledby="cost-heading" className={PANEL}>
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <ButtonLink
                href="/tools/crm-cost-calculator/?from=scorecard"
                variant="outline"
                size="sm"
                onClick={() =>
              ...
              <ButtonLink
                href="/tools/crm-tco-calculator/?from=scorecard"
                variant="outline"
                size="sm"
              >
                Open CRM TCO Calculator
              </ButtonLink>
            </div>
            <p className="mt-1 text-sm text-[var(--sg-color-text-muted)]">
          ...
          <section aria-labelledby="next-heading" className={PANEL}>
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              <ButtonLink
                href="/tools/crm-implementation-planner/?from=scorecard"
                variant="outline"
              >
                Build implementation plan
              </ButtonLink>
              <ButtonLink href="/tools/crm-finder/" variant="outline">
                CRM Finder
              </ButtonLink>
              <ButtonLink
                href="/tools/crm-requirements-builder/"
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <Button
                ...
                size="sm"
                onClick={() => {
                  const empty = resetVendorScorecard("crm");
                  setState(
                    touchVendorScorecard(empty, {
                      criteria: generateCriteriaFromProfile(profile),
                      researchAcknowledgedAt: research.generatedAt,
                    }),
                  );
                }}
        ...
        <aside className="hidden lg:block">
          <div className="sticky top-24 space-y-4 rounded-[var(--sg-radius-xl)] border border-[var(--sg-color-border)] bg-[var(--sg-color-surface)] p-5 shadow-[var(--sg-shadow-md)]">
            ...
            <div className="flex flex-col gap-2 border-t border-[var(--sg-color-border)] pt-3">
              ...
              <ButtonLink href="/tools/crm-finder/" variant="ghost" size="sm">
                CRM Finder
              </ButtonLink>
              <ButtonLink
                href="/tools/crm-cost-calculator/?from=scorecard"
