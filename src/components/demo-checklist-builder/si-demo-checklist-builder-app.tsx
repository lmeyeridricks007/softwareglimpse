...
import {
  createSeededSiDemoChecklistSession,
  loadSiDemoChecklistSession,
  resetSiDemoChecklistSession,
  saveSiDemoChecklistSession,
  touchSiDemoChecklistSession,
} from "@/services/demo-checklist-builder/si-persistence";
...
type Props = {
  ...
  runtime?: {
    categorySlug: string;
    loadSession: () => CrmDemoChecklistSession | null;
    saveSession: (session: CrmDemoChecklistSession) => void;
    createSeeded: () => CrmDemoChecklistSession;
    resetSession: () => CrmDemoChecklistSession;
    loadProfile: () => DecisionProfile | null;
  };
};

const SI_DEMO_RUNTIME = {
  categorySlug: "sales-intelligence",
  loadSession: loadSiDemoChecklistSession,
  saveSession: saveSiDemoChecklistSession,
  createSeeded: createSeededSiDemoChecklistSession,
  resetSession: resetSiDemoChecklistSession,
  loadProfile: loadSiDecisionProfile,
} as const;

export function SiDemoChecklistBuilderApp({
  title = "Sales Intelligence Demo Checklist Builder",
  description = "Build a reusable demo agenda and evaluation workbook — same script for every vendor, with per-vendor scoring.",
  titleElement = "none",
  runtime = SI_DEMO_RUNTIME,
}: Props) {
  ...
  useEffect(() => {
    try {
      let next =
        runtime.loadSession() ?? runtime.createSeeded();
      ...
      if (!startedRef.current) {
        ...
        track({
          name: "tool_start",
          properties: { tool: `${runtime.categorySlug}-demo-checklist-builder`, category: runtime.categorySlug },
        });
      }
    } catch (err) {
      console.error("[si-demo-checklist-builder] hydrate failed", err);
      setSession(runtime.createSeeded());
    } finally {
      setHydrated(true);
    }
  }, []);
  ...
  const patchDraft: DemoDraftPatch = useCallback((updater) => {
    setSession((prev) =>
      touchSiDemoChecklistSession(prev, {
        ...
        <Button
          ...
          onClick={() => {
            if (
              ...
              setSession(runtime.resetSession());
            }
          }}
        >
          Start over
        ...
        <Card className="px-4 py-5 sm:px-6 sm:py-6">
          ...
          {onResults && !isLoading ? (
            <DemoResults
              ...
              scorecardCategorySlug={runtime.categorySlug}
            />
          ) : null}
