/**
 * Sales Intelligence Requirements Builder — interactive wizard.
 * Persistence: localStorage `sg-si-decision-profile-v1` only.
...
import {
  ...
  SI_CURRENT_STATE_OPTIONS as CURRENT_STATE_OPTIONS,
  SI_INTEGRATION_OPTIONS,
  SI_SECURITY_REQUIREMENT_SLUGS as SECURITY_REQUIREMENT_SLUGS,
  SI_TEAM_OPTIONS as TEAM_OPTIONS,
...
function bootstrapSiRequirementsWizard(
  searchParams: URLSearchParams,
  stages: readonly { id: string }[],
): WizardBootstrap {
  const stored = loadSiDecisionProfile();
  const base = stored ?? createEmptySiDecisionProfile();
...
export function SiRequirementsBuilderApp({
  relatedGuides = [],
  faqItems,
  titleElement = "h1",
}: Props) {
  const searchParams = useSearchParams();
  const def = siRequirementsBuilderDefinition;
  const stages = def.stages;
  const headingId = useId();
  const wizardRef = useRef<HTMLElement | null>(null);

  const [initialWizard] = useState(() =>
    bootstrapSiRequirementsWizard(searchParams, stages),
  );
  ...
  const [profile, setProfile] = useState<SiDecisionProfile>(
    initialWizard.profile,
  );
  const currentSearchKey = searchParams.toString();
  if (currentSearchKey !== sessionKey) {
    const next = bootstrapSiRequirementsWizard(searchParams, stages);
  ...
  const persist = useCallback((next: SiDecisionProfile) => {
    saveSiDecisionProfile(next);
    setProfile(next);
  }, []);

  const updateProfile = useCallback(
    (patch: Partial<SiDecisionProfile>) => {
      const next = touchSiDecisionProfile(profile, {
        ...patch,
  ...
  const startBuilding = () => {
    ...
    if (!startedTracked.current) {
      ...
      track({
        ...
        properties: { category: "sales-intelligence" },
      });
  ...
  const handleReset = () => {
    const empty = resetSiDecisionProfile();
    setProfile(empty);
    setStepIndex(0);
    setMaxStepIndex(0);
    setStarted(false);
  ...
  const exportText = async (kind: "copy" | "json" | "csv") => {
    ...
    if (kind === "copy") {
      ...
      a.download =
        kind === "json"
          ? "si-requirements-profile.json"
          : "si-requirements-checklist.csv";
      a.click();
    ...
    track({
      ...
      properties: { format: kind, category: "sales-intelligence" },
    });
  ...
  const exportPdf = () => {
    void downloadProfilePdf(profile)
      .then(() => {
        track({
          name: "requirements_exported",
          properties: { format: "pdf", category: "sales-intelligence" },
        });
  ...
  const exportExcel = () => {
    downloadBlob(
      ...
      "si-requirements-profile.xls",
      "application/vnd.ms-excel",
    );
    track({
      name: "requirements_exported",
      properties: { format: "excel", category: "sales-intelligence" },
    });
  ...
  return (
    <div className="space-y-10">
      <RequirementsBuilderHero
        ...
        eyebrow="Sales Intelligence Requirements Builder"
        headline="Build your sales intelligence requirements"
        description="Answer a few questions about your outbound motion, data needs and priorities. We'll turn them into a structured sales intelligence requirements profile you can reuse across SoftwareGlimpse tools."
        readinessNote={null}
        exampleTitle="Your SI Requirements"
        exampleUseCases={[
          { label: "Prospecting", done: true },
          { label: "Data enrichment", done: true },
          { label: "List building", done: true },
        ]}
        exampleMustHave={[
          { label: "Verified emails", done: true },
          { label: "CRM sync", done: true },
          { label: "Regional coverage", done: true },
        ]}
        exampleNiceToHave={[
          { label: "Intent signals", done: false },
          { label: "AI assistance", done: false },
        ]}
        exampleIntegrations={[
          { label: "Salesforce", done: true },
          { label: "HubSpot", done: true },
        ]}
      />
...
function BusinessStep({
  ...
  profile: SiDecisionProfile;
  onChange: (patch: Partial<SiDecisionProfile>) => void;
}) {
  ...
  return (
    <StepShell
      ...
      description="Only details that help evaluate sales intelligence fit downstream."
    >
      ...
      <fieldset className="grid gap-4 sm:grid-cols-2">
        <legend className="sr-only">Company size and seats / users</legend>
        <label className="block text-sm">
          <span className="font-medium text-[var(--sg-color-navy)]">
            Company size
          </span>
      ...
      <fieldset>
        <legend className="text-sm font-medium text-[var(--sg-color-navy)]">
          Who will primarily use this tool?
        </legend>
...
function UseCasesStep({
  ...
  const options = listSelectableSiUseCases();
  ...
  return (
    <StepShell
      title="What do you need sales intelligence for?"
      description="Select use cases from the SoftwareGlimpse sales intelligence taxonomy. Mark one as primary."
    >
...
function CapabilitiesStep({
  ...
  const { recommended, other } = deriveSiCapabilitiesFromUseCases(
  ...
  const allCaps = listSelectableSiCapabilities();
  ...
  return (
    <StepShell
      title="Which sales intelligence capabilities matter?"
...
function RequirementsStep({
  ...
  return (
    <StepShell
      ...
      description="Requirements are derived from your capabilities and use cases via the sales intelligence knowledge graph."
    >
...
function FeaturesStep({
  ...
  const derived = useMemo(
    () =>
      deriveSiFeaturesFromRequirements(profile.requirements, profile.features),
  ...
  const optional = listOptionalSiDirectFeatures().filter(
...
function IntegrationsStep({
  ...
  const options = SI_INTEGRATION_OPTIONS;

  return (
    <StepShell
      title="What must the tool connect to?"
      description="CRM and productivity integrations commonly used with sales intelligence tools."
    >
      <div className="grid gap-3 sm:grid-cols-2">
        {options.map((opt) => {
...
function SecurityStep({
  ...
  const ensureSecurityReqs = () => {
    ...
    const additions = SECURITY_REQUIREMENT_SLUGS.filter(
      (slug) => !existing.has(slug) && resolveSiRequirementMeta(slug),
    ).map((slug) => ({
      id: slug,
  ...
  return (
    <StepShell
      ...
      description="Compliance, credits, and export rights from the SI taxonomy. We do not imply regulatory compliance."
    >
      <div className="space-y-3">
        {SECURITY_REQUIREMENT_SLUGS.map((slug) => {
          const meta = resolveSiRequirementMeta(slug);
            ...
            <SelectableCard
              ...
              onClick={() =>
                onChange({
                  budget: {
                    ...profile.budget,
                    band: opt.value as SiDecisionProfile["budget"]["band"],
                  },
                })
              }
            />
                ...
                onChange({
                  implementation: {
                    ...
                    complexity: opt.value as NonNullable<
                      SiDecisionProfile["implementation"]["complexity"]
                    >,
                  },
                })
              }
...
function PrioritizeStep({
  ...
  const byCapability = new Map<
    string,
    SiDecisionProfile["requirements"]
  >();
