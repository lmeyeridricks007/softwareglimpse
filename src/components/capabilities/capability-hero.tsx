...
export type CapabilityHeroStat = {
  label: string;
  href?: string;
  icon?: "products" | "capabilities" | "independent" | "methodology";
};
...
type Props = {
  title: string;
  description?: string;
  stats?: CapabilityHeroStat[];
  className?: string;
};

export function CapabilityHubHero({
  title,
  description,
  stats = [],
  className,
    ...
    >
      <div>
        ...
        <p className="mt-4 text-xs text-[var(--sg-color-text-muted)]">
          Affiliate relationships never set capability guidance.{" "}
          <Link
        ...
        <div className="min-w-0 flex-1 rounded-[var(--sg-radius-lg)] border border-[var(--sg-color-border)] bg-[var(--sg-color-surface)] p-3">
          ...
          <div className="mt-4 rounded-[var(--sg-radius-md)] border border-[var(--sg-color-success)]/30 bg-[var(--sg-color-success-soft)]/70 px-3 py-2">
            ...
            <p className="mt-0.5 text-[10px] text-[var(--sg-color-text-muted)]">
              Matched to capabilities — not invented metrics
            </p>
          </div>
        </div>
      </div>
...
export function CapabilityDetailHero({
  ...
  return (
    ...
    >
      ...
      {heroVisual ? (
        <figure className="mt-8 overflow-hidden rounded-[var(--sg-radius-xl)] border border-white bg-[var(--sg-color-surface)] shadow-[0_12px_40px_rgb(15_23_42/0.08)]">
          <Image
            src={heroVisual.src}
            alt={heroVisual.alt}
            width={1536}
