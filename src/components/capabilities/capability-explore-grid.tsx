...
import { iconForCapabilitySlug, toneForCapabilitySlug } from "./capability-icons";
...
export function CapabilityExploreGrid({
  ...
  return (
    <section
      ...
      aria-labelledby="capability-explore-heading"
    >
      <h2
        id="capability-explore-heading"
      ...
      <ul className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {items.map((item) => {
          const Icon = item.icon ?? iconForCapabilitySlug(item.slug);
          const tone = toneForCapabilitySlug(item.slug);
          return (
            <li key={item.slug}>
...
export function CapabilityAudienceRow({
  title = "Capabilities by audience",
  items,
  className,
}: {
  title?: string;
  ...
  return (
    <section
      className={cn(className)}
      aria-labelledby="capability-audience-heading"
    >
      <h2
        id="capability-audience-heading"
        className="font-[family-name:var(--font-display)] text-[length:var(--sg-text-h2)] font-semibold text-[var(--sg-color-text)]"
      >
        {title}
      </h2>
