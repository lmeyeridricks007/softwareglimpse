...
export type BusinessTypeHubItem = {
  slug: string;
  name: string;
...
type Props = {
  title: string;
  items: BusinessTypeHubItem[];
  className?: string;
};

export function CategoryBusinessTypes({ title, items, className }: Props) {
  return (
    <section
      id="business-types"
    ...
    >
      ...
      {items.length === 0 ? (
        <p className="mt-5 max-w-2xl text-sm text-[var(--sg-color-text-muted)] sm:text-base">
          Business type pages are still being added. Browse{" "}
          <Link
            href="/for/"
        ...
        <ul className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item, index) => {
            const Icon = ICONS[item.slug] ?? Users;
