import type { ReactNode } from "react";
import Link from "next/link";
import { createElement } from "react";
import { iconForCapabilitySlug } from "@/components/capabilities/capability-icons";
import { hubToneClassForSlug, withSingleArrow } from "@/components/category/hub-icons";
import { Card } from "@/components/ui/card";
import { cn } from "@/lib/cn";

export type CapabilityHubItem = {
  slug: string;
  name: string;
  description: string | null;
  href: string;
};

type Props = {
  title: string;
  items: CapabilityHubItem[];
  viewAllHref?: string;
  emptyHint?: ReactNode;
  className?: string;
};

export function CategoryCapabilities({
  title,
  items,
  viewAllHref = "/capabilities/",
  emptyHint,
  className,
}: Props) {
  return (
    <section
      id="capabilities"
      aria-labelledby="capabilities-heading"
      className={cn(
        "scroll-mt-28 rounded-[var(--sg-radius-xl)] bg-[var(--sg-color-surface-muted)] px-5 py-8 sm:px-8",
        className,
      )}
    >
      <div className="flex flex-wrap items-end justify-between gap-3">
        <h2
          id="capabilities-heading"
          className="font-[family-name:var(--font-display)] text-[length:var(--sg-text-h2)] font-semibold text-[var(--sg-color-text)]"
        >
          {title}
        </h2>
        {items.length > 0 && viewAllHref ? (
          <Link
            href={viewAllHref}
            className="text-sm font-medium text-[var(--sg-color-primary)] underline-offset-2 hover:underline"
          >
            {withSingleArrow("View all capabilities")}
          </Link>
        ) : null}
      </div>
      {items.length === 0 ? (
        <p className="mt-5 max-w-2xl text-sm text-[var(--sg-color-text-muted)] sm:text-base">
          {emptyHint ?? (
            <>
              Capability pages for this category are still being added. Browse{" "}
              <Link
                href={viewAllHref}
                className="font-medium text-[var(--sg-color-primary)] underline-offset-2 hover:underline"
              >
                all capabilities
              </Link>{" "}
              or start with Finder above.
            </>
          )}
        </p>
      ) : (
        <ul className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {items.map((item) => {
            const Icon = iconForCapabilitySlug(item.slug);
            return (
              <li key={item.slug}>
                <Link href={item.href} className="group block h-full">
                  <Card
                    variant="interactive"
                    className="flex h-full flex-col p-4"
                  >
                    <span
                      className={cn(
                        "inline-flex size-10 items-center justify-center rounded-[var(--sg-radius-md)] border",
                        hubToneClassForSlug(item.slug),
                      )}
                    >
                      {createElement(Icon, {
                        className: "size-5",
                        "aria-hidden": true,
                      })}
                    </span>
                    <p className="mt-3 font-semibold text-[var(--sg-color-text)] group-hover:text-[var(--sg-color-primary)]">
                      {item.name}
                    </p>
                    {item.description ? (
                      <p className="mt-1.5 flex-1 text-sm text-[var(--sg-color-text-muted)]">
                        {item.description}
                      </p>
                    ) : null}
                    <span className="mt-3 text-sm font-medium text-[var(--sg-color-primary)]">
                      {withSingleArrow("Explore")}
                    </span>
                  </Card>
                </Link>
              </li>
            );
          })}
        </ul>
      )}
    </section>
  );
}
