/**
 * CRM RFP / Vendor Brief Builder interactive app.
 * Persistence: localStorage `sg-crm-rfp-brief-v1`
 */
"use client";
...
export function CrmRfpBuilderApp() {
  ...
  useEffect(() => {
    const stored = loadCrmRfpSession();
    // eslint-disable-next-line react-hooks/set-state-in-effect -- localStorage hydration
    if (stored) setSession(stored);
    else setSession(createSeededCrmRfpSession());
    setHasProfile(Boolean(loadCrmDecisionProfile()));
    setHydrated(true);
  }, []);

  useEffect(() => {
    if (!hydrated) return;
    saveCrmRfpSession(session);
  }, [session, hydrated]);
    ...
    (updater: (prev: CrmRfpDraft) => CrmRfpDraft) => {
      setSession((prev) => {
        ...
        return detectPostIssueChanges(
          touchCrmRfpSession(prev, { draft: nextDraft }),
        );
      });
    },
    [],
  ...
  const go = (next: RfpWizardStep) => {
    ...
    persist(setWizardStep(session, next));
  ...
  const importProfileRequirements = () => {
    ...
    if (!profile) return;
    patchDraft((d) => applyProfileToDraft(d, profile));
  ...
  const importLibrary = () => {
    patchDraft((d) => ({
      ...d,
      requirements: requirementsFromLibrary(),
    }));
  ...
  const importIntegrationsOnly = () => {
    ...
    patchDraft((d) => ({
      ...d,
      integrations: integrationsFromProfile(profile),
    }));
  };
  ...
  const generate = () => {
    ...
    next = setWizardStep(next, "results");
    ...
    <div className="grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)_280px]">
      ...
      >
        <Card className="sticky top-4 space-y-4 p-4">
          ...
          >
            How to write a CRM RFP that gets useful answers →
          </Link>
        </Card>
      </nav>
