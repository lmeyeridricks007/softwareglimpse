 ...
 * Persistence: localStorage `sg-si-plan-selector-v1`
...
const STORAGE_KEY = "sg-si-plan-selector-v1";

const STAGES = [
  { id: "crm", label: "Product" },
  { id: "team", label: "Team" },
...
export type PlanSelectorAppConfig = {
  storageKey: string;
  productLabel: string;
  finderHref: string;
  finderLabel: string;
  startedEvent: AnalyticsEventName;
  vendorEvent: AnalyticsEventName;
  generatedEvent: AnalyticsEventName;
};

const SI_PLAN_SELECTOR_CONFIG: PlanSelectorAppConfig = {
  storageKey: STORAGE_KEY,
  productLabel: "product",
  finderHref: "/tools/sales-intelligence-finder/",
  finderLabel: "Sales Intelligence Finder",
  startedEvent: "si_plan_selector_started",
  vendorEvent: "si_plan_vendor_selected",
  generatedEvent: "si_plan_recommendation_generated",
};

type Props = {
  snapshots: PricingSnapshot[];
  config?: PlanSelectorAppConfig;
};
...
function readDraft(storageKey: string): CrmPlanSelectorAnswers | null {
...
export function SiPlanSelectorApp({ snapshots, config = SI_PLAN_SELECTOR_CONFIG }: Props) {
  ...
  useEffect(() => {
    const draft = readDraft(config.storageKey);
    ...
    setAnswers(next);
    setHydrated(true);
    track({ name: config.startedEvent, properties: { category: config.productLabel } });
        ...
        <div className="min-w-0">
          ...
          !canUseVendor ? (
            <p className="mt-4 text-sm text-[var(--sg-color-warning)]" role="status">
              Plan comparison is not yet available for this {config.productLabel}. Try{" "}
              <a
                href={config.finderHref}
        ...
        <aside className="min-w-0">
          ...
          >
            ...
            <ul className="mt-2 space-y-1">
              ...
              <li>
                <a href={config.finderHref} className="hover:underline">
                  Need help choosing {config.productLabel}?
                </a>
              </li>
            </ul>
          </nav>
