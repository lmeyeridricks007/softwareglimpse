/**
 * Sales Intelligence Finder interactive wizard.
 *
 * Persistence: localStorage only (`sg-si-finder-v1`). No answer URLs.
 * Scoring: pure recommendSalesIntelligence on server-serialized snapshots.
 */
"use client";
...
import { siFinderDefinition } from "@/components/finder/framework";
import {
  BUDGET_OPTIONS,
  BUSINESS_TYPE_OPTIONS,
  COMPANY_SIZE_OPTIONS,
  EASE_OPTIONS,
  SI_CAPABILITY_OPTIONS,
  SI_FINDER_QUESTIONS,
  SI_FINDER_STAGES,
  SI_INTEGRATION_OPTIONS,
  SI_USE_CASE_OPTIONS,
  labelForOption,
  siStageIndexForQuestion,
  siFirstQuestionIndexForStage,
  type FinderQuestion,
} from "./si-finder-questions";
...
const STORAGE_KEY = siFinderDefinition.storageKey;
...
export function SiFinderApp({
  ...
  function goNext() {
    ...
    ensureStarted();
    track({
      name: "si_finder_step_completed",
      properties: { step: question.id, index: stepIndex },
    });
    if (stepIndex >= questions.length - 1) {
      setPhase("review");
      setMaxStageIndex(SI_FINDER_STAGES.length - 1);
      return;
    }
    setStepIndex((i) => i + 1);
  }
  ...
  function buildCompleteAnswers(): SiFinderAnswers | null {
    if (
  ...
  function runRecommendations() {
    ...
    if (!complete) return;
    const criteria = normalizeSiFinderAnswers(complete, siFinderConfig);
    const outcome = recommendSalesIntelligence(
      criteria,
      snapshots,
      siFinderConfig,
    );
  ...
  function restart() {
    ...
    track({ name: "si_finder_restarted" });
  }
  ...
  function goToStage(stageId: string) {
    ...
    const qIndex = siFirstQuestionIndexForStage(stageId);
    if (qIndex < 0) return;
    resetReveal();
    setPhase("questions");
    setStepIndex(qIndex);
  ...
  const stageStepTotal = SI_FINDER_STAGES.length;
  const stageLabel = SI_FINDER_STAGES[stageIndex]?.label ?? "Business";
  const progressPct =
  ...
  if (phase === "questions" && question) {
    ...
    main = (
      <div className="rounded-[var(--sg-radius-xl)] border border-[var(--sg-color-border)] bg-[var(--sg-color-surface)] p-5 shadow-[var(--sg-shadow-md)] sm:p-7">
        ...
        <dl className="mt-6 space-y-3 text-sm">
          ...
          <ReviewRow
            label="Primary job"
            value={labelForOption(SI_USE_CASE_OPTIONS, answers.primaryUseCaseSlug)}
          />
          <ReviewRow
            ...
            value={
              requiredFeatures.length > 0
                ? requiredFeatures
                    .map((slug) => labelForOption(SI_CAPABILITY_OPTIONS, slug))
                    .join(", ")
                : "None selected"
            }
          />
            ...
            value={
              (answers.preferredIntegrationSlugs ?? []).length > 0
                ? (answers.preferredIntegrationSlugs ?? [])
                    .map((slug) => labelForOption(SI_INTEGRATION_OPTIONS, slug))
                    .join(", ")
                : "None selected"
            }
          />
    ...
    main = (
      <ResultsLoadingState
        title="Building your sales intelligence shortlist…"
        description="Matching your requirements against SoftwareGlimpse SI research."
      />
    );
  } else {
    main = (
      <div
        id="si-finder-results"
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-[var(--sg-color-primary)]">
              Your sales intelligence matches
            </p>
            ...
            <p className="mt-2 text-sm text-[var(--sg-color-text-muted)]">
              {emptyReason === "all-excluded"
                ...
                : emptyReason === "insufficient-data"
                  ...
                  : "No sales intelligence products were available to score."}
            </p>
            <Button
              type="button"
              className="mt-4"
            ...
            {selectedSlugs.length >= 2 ? (
              compareHref ? (
                <Link
                  ...
                  onClick={() =>
                    track({
                      name: "si_finder_comparison_clicked",
        ...
        <div className="flex flex-wrap gap-3 border-t border-[var(--sg-color-border)] pt-6">
          <Link
            href={siFinderDefinition.calculatorHref}
            onClick={() => track({ name: "si_finder_guide_clicked" })}
          ...
          >
            Browse best sales intelligence software →
          </Link>
          <Button
            type="button"
            variant="outline"
    ...
    >
      ...
      {showProgressSidebar ? (
        <FinderProgressPanel
          ...
          criteria={siFinderDefinition.matchCriteria}
        />
      ) : null}
    </div>
  );
  ...
  return (
    <div className="rounded-[var(--sg-radius-lg)] border border-[var(--sg-color-border)] bg-[var(--sg-color-surface-muted)]/80 p-4 sm:p-5">
      ...
      <dl className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 text-sm">
        ...
        <div>
          <dt className="text-[var(--sg-color-text-muted)]">Primary job</dt>
          <dd className="font-medium">
            {labelForOption(SI_USE_CASE_OPTIONS, answers.primaryUseCaseSlug) ??
              "—"}
          </dd>
        </div>
        <div>
          ...
          <dd className="mt-1 flex flex-wrap gap-1.5">
            {(answers.requiredFeatureSlugs ?? []).length > 0 ? (
              (answers.requiredFeatureSlugs ?? []).map((slug) => (
                ...
                >
                  <Check
                    className="size-3 text-[var(--sg-color-success)]"
                    aria-hidden
                  />
                  {labelForOption(SI_CAPABILITY_OPTIONS, slug)}
                </span>
              ))
            ) : (
