...
import { READINESS_DIMENSIONS } from "@/services/readiness-assessment/catalog";
import {
...
import { runFullAssessment } from "@/services/readiness-assessment/findings";
import {
  assessCrmReadiness,
  estimateMinutesRemaining,
  getVisibleQuestions,
} from "@/services/readiness-assessment/score";
import { seedDecisionProfileFromReadiness } from "@/services/readiness-assessment/to-profile";
import {
...
export function CrmReadinessAssessmentApp() {
  ...
  useEffect(() => {
    let cancelled = false;
    const stored = loadCrmReadinessSession();
    ...
    const id = window.setTimeout(() => {
      ...
      setHydrated(true);
      track({ name: "crm_readiness_started" });
    }, 0);
    return () => {
      cancelled = true;
  ...
  const provisional = useMemo(() => {
    ...
    return assessCrmReadiness(session);
  }, [session]);
  ...
  const startAssessment = () => {
    update(
      touchCrmReadinessSession(session, {
        wizardStep: "context",
        status: "in-progress",
      }),
    );
  ...
  const finishAssessment = () => {
    update(touchCrmReadinessSession(session, { wizardStep: "computing" }));
    startReveal(() => {
      ...
      const profile = loadCrmDecisionProfile();
      saveCrmDecisionProfile(
        seedDecisionProfileFromReadiness(completed, profile),
      );
  ...
  const restart = () => {
    resetReveal();
    const empty = resetCrmReadinessSession();
    setImportedIds([]);
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          {session.lastResult ? (
            <Button
              ...
              onClick={() =>
                update(
                  touchCrmReadinessSession(session, { wizardStep: "results" }),
                )
              }
            >
              View last results
  ...
  if (session.wizardStep === "context") {
    return (
      <ContextStep
        ...
        onBack={() =>
          update(touchCrmReadinessSession(session, { wizardStep: "landing" }))
        }
  ...
  const minutes = estimateMinutesRemaining(session);
    ...
    >
      <DimensionStep
        ...
        onBack={() => {
          if (dimIndex === 0) {
            update(
              touchCrmReadinessSession(session, { wizardStep: "context" }),
            );
          } else {
