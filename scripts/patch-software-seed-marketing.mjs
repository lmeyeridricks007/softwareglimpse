#!/usr/bin/env node
/**
 * Inserts soft() entries from scripts/_marketing-batch-seed-snippet.ts into
 ...
 *   node scripts/patch-software-seed-marketing.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_marketing-batch-seed-snippet.ts");
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-marketing-batch.mjs`,
    );
    process.exit(1);
  }
