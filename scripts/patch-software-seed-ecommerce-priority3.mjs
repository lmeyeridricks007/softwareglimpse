#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_ecommerce-priority3-seed-snippet.ts into
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_ecommerce-priority3-seed-snippet.ts");
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-ecommerce-priority3-batch.mjs`,
    );
    process.exit(1);
  }
