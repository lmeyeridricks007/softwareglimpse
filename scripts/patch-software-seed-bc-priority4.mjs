#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_bc-priority4-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-bc-priority4-batch.mjs
 *   node scripts/patch-software-seed-bc-priority4.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_bc-priority4-seed-snippet.ts");

/** Priority-3 left ~114; refuse if seed looks truncated. */
const MIN_EXPECTED_ENTRIES = 110;
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-bc-priority4-batch.mjs`,
    );
    process.exit(1);
  }
