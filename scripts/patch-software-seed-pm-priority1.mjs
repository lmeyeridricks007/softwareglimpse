#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_pm-priority1-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-pm-priority1-batch.mjs
 *   node scripts/patch-software-seed-pm-priority1.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_pm-priority1-seed-snippet.ts");
...
function main() {
  ...
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-pm-priority1-batch.mjs`,
    );
    process.exit(1);
  }
