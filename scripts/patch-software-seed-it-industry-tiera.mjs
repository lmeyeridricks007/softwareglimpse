#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_it-industry-tiera-seed-snippet.ts into
 * src/data/seed/software.ts before `export const softwareSeed`
 * (closing `];` of softwareSeedRaw).
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-it-industry-tiera-batch.mjs
 *   node scripts/patch-software-seed-it-industry-tiera.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_it-industry-tiera-seed-snippet.ts");

/** Prior IT batches left 230+; refuse if seed looks truncated. */
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-it-industry-tiera-batch.mjs`,
    );
    process.exit(1);
  }
