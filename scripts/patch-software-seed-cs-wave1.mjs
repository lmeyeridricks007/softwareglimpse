#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_cs-wave1-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Re-homes tidio: replaces existing soft-tidio block with CS-primary snippet
 * (primaryCategorySlug customer-service, live-chat use cases).
 *
 * Append-only for new slugs; idempotent. Usage:
 *   node scripts/onboard-cs-wave1-batch.mjs
 *   node scripts/patch-software-seed-cs-wave1.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_cs-wave1-seed-snippet.ts");

/** Prior batches left ~158; refuse if seed looks truncated. */
const MIN_EXPECTED_ENTRIES = 158;

/** Slugs that already exist in seed and must be replaced, not skipped. */
const REPLACE_SLUGS = new Set(["tidio", "freshservice"]);
...
function findSoftBlockBounds(source, slug) {
  const marker = `id: "soft-${slug}"`;
...
function replaceSoftBlock(source, slug, newBlock) {
  const bounds = findSoftBlockBounds(source, slug);
  if (!bounds) {
    console.warn(`${slug}: existing soft() block not found — will append if new`);
    return { source, replaced: false };
  }
  const next =
    source.slice(0, bounds.blockStart) +
    newBlock +
    "\n" +
    source.slice(bounds.blockEnd);
  console.log(`${slug}: replaced existing soft() with CS Wave-1 snippet (primaryCategorySlug customer-service)`);
...
function main() {
  ...
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-cs-wave1-batch.mjs`,
    );
    process.exit(1);
  }
  ...
  for (const block of blocks) {
    ...
    if (slug) blocksBySlug.set(slug, block);
  }

  let replaced = 0;
  for (const slug of REPLACE_SLUGS) {
    const block = blocksBySlug.get(slug);
    if (!block) continue;
    const result = replaceSoftBlock(source, slug, block);
    source = result.source;
    if (result.replaced) {
      replaced++;
      blocksBySlug.delete(slug);
    }
  }
  ...
  if (replaced) console.log(`Replaced ${replaced} existing entr(y/ies)`);
  if (toInsert.length) {
