#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_ecommerce-wave1-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-ecommerce-wave1-batch.mjs
 *   node scripts/patch-software-seed-ecommerce-wave1.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_ecommerce-wave1-seed-snippet.ts");

const MIN_EXPECTED_ENTRIES = 170;
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-ecommerce-wave1-batch.mjs`,
    );
    process.exit(1);
  }
