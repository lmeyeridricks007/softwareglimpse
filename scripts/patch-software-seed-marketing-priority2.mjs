#!/usr/bin/env node
/**
 * Inserts soft() entries from scripts/_marketing-priority2-seed-snippet.ts into
 ...
 *
 * Usage:
 *   node scripts/onboard-marketing-priority2-batch.mjs
 *   node scripts/patch-software-seed-marketing-priority2.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_marketing-priority2-seed-snippet.ts");
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-marketing-priority2-batch.mjs`,
    );
    process.exit(1);
  }
