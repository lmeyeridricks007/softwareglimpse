#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_hr-wave1-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Also wires LearnWorlds secondaryCategorySlugs: ["hr"] + HR use cases
 * without overwriting marketing primary fields, assessment, or review.
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-hr-wave1-batch.mjs
 *   node scripts/patch-software-seed-hr-wave1.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_hr-wave1-seed-snippet.ts");
...
/**
 * Wire LearnWorlds as marketing-primary + HR secondary without destroying
...
function patchLearnworldsSecondary(source) {
  ...
  if (start < 0) {
    console.warn("LearnWorlds soft() entry not found — skip secondary HR wiring");
  ...
  if (blockStart < 0 || blockEnd < 0) {
    console.warn("Could not bound LearnWorlds soft() block");
    return { source, changed: false };
  }
  ...
  if (!changed) {
    console.log("LearnWorlds: secondary HR already wired");
    return { source, changed: false };
  }

  const next = source.slice(0, blockStart) + block + source.slice(blockEnd);
  console.log(
    'LearnWorlds: added secondaryCategorySlugs: ["hr"] + HR useCaseSlugs (marketing primary preserved)',
  );
...
function main() {
  ...
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-hr-wave1-batch.mjs`,
    );
    process.exit(1);
  }
