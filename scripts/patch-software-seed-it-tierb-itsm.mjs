#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_it-tierb-itsm-seed-snippet.ts into
 * src/data/seed/software.ts before `export const softwareSeed`
 * (closing `];` of softwareSeedRaw).
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-it-tierb-itsm-batch.mjs
 *   node scripts/patch-software-seed-it-tierb-itsm.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_it-tierb-itsm-seed-snippet.ts");
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-it-tierb-itsm-batch.mjs`,
    );
    process.exit(1);
  }
