#!/usr/bin/env node
/**
 * Shared Project Management & Productivity onboarding runtime.
 * Adapted from bc-onboard-runtime.mjs for project-management-editorial v1.0.0.
 */
...
export let BATCH_LABEL = "Project-management Wave-1";
export let JOB_TAG = "project-management-wave1";

export function configurePmRuntime(opts = {}) {
...
export const PM_CRITERION_WEIGHTS = {
  "ease-of-use": 12,
  "work-planning": 15,
...
export function weightedScore(scores) {
  const total = PM_CRITERIA.reduce(
    (sum, criterion) => sum + scores[criterion] * PM_CRITERION_WEIGHTS[criterion],
    0,
  );
...
export function buildProductFixture(p) {
  const featureLines = PM_FEATURES.map(
...
export function supportingFeatureForCriterion(criterionSlug) {
  const map = {
    "ease-of-use": "task-boards",
    "work-planning": "timeline-gantt",
    "automation-workflows": "automations-workflows",
    collaboration: "docs-collaboration",
    integrations: "integrations-ecosystem",
    reporting: "reporting-dashboards",
    scalability: "workload-resources",
    "value-for-money": "task-boards",
    "ai-capabilities": "ai-assistance",
  };
  return map[criterionSlug] ?? "task-boards";
}

export function buildEnrichment(p) {
  const slugs = planSlugs(p);
  const featureSupport = PM_FEATURES.map((featureSlug) => ({
    featureSlug,
  ...
  const editorialFit = (p.businessSizeSlugs ?? []).map((businessSizeSlug) => {
    ...
    return {
      ...
      rationale: `${p.name} fit for ${businessSizeSlug} ${primaryTeamType} teams in the ${p.jobCluster} job cluster, from first-party positioning and ${BATCH_LABEL} research${
        p.membershipRole === "adjacent"
          ? " (adjacent tool — not a work-OS peer)"
          : ""
      }.`,
      isEditorial: true,
    };
...
export function buildFacts(p) {
  ...
  const facts = [
    ...
    factBase(
      ...
      p.jobCluster,
      productSrc,
      `Project-management job cluster: ${p.jobCluster}`,
      "JOB_CLUSTER",
    ),
    factBase(
      p,
  ...
  for (const featureSlug of PM_FEATURES) {
...
export function buildCriterionAssessments(p) {
  return PM_CRITERIA.map((criterionSlug) => {
...
const OVERALL_RATIONALE =
  "Weighted average of the nine project-management editorial criteria using the category weights (work planning 15, automation & workflows 14, ease of use 12, collaboration 12, integrations 12, reporting 10, value 10, scalability 9, AI 6), rounded to 1 decimal. Products are scored inside their job cluster — work OS / PM peers, timeline specialists, and adjacent PDF / remote-access / desktop-workspace tools are not forced into one undifferentiated ranking. Not a hands-on lab score, and affiliate economics are excluded.";
...
export function buildAssessment(p) {
  ...
  const adjacentBit =
    p.membershipRole === "adjacent"
      ? " Marked adjacent — not a work-OS / project-management peer for undifferentiated best-page ranking."
      : "";
  return {
    id: `assessment-${p.slug}-project-management-v1`,
    productSlug: p.slug,
    methodologySlug: "project-management-editorial",
    methodologyVersion: "1.0.0",
    status: "approved",
    verdict: `${p.name}: ${p.whoShouldChoose}${adjacentBit} Scores use the project-management editorial methodology from first-party research as of 2026-08-17 — not hands-on product testing.`,
    strengths: p.pros.slice(0, 5),
    weaknesses: p.cons.slice(0, 5),
    bestFor: p.bestFor,
    notIdealFor: p.notIdealFor,
    tradeoffs: [
      "Seat minimums and AI credit bundles vs published per-seat floors",
      "Work OS breadth vs specialist timeline / PDF / remote / desktop depth",
      "Automation and reporting depth vs ease of use for small teams",
      "Included AI credits vs paid AI add-ons or credit top-ups",
    ],
    recommendation: `${p.whoShouldChoose} ${p.whoShouldConsiderAlternatives}`,
    editorialNotes: `${BATCH_LABEL} batch. Approved on project-management-editorial v1.0.0. membershipRole=${p.membershipRole}. jobCluster=${p.jobCluster}. handsOnTesting=false. Affiliate economics excluded.${
...
const CLUSTER_LABELS = {
  "work-os": "a work OS / work management platform",
  timeline: "a specialist timeline / Gantt presentation tool",
  "document-pdf": "an adjacent document / PDF productivity tool",
  "remote-access": "an adjacent remote access / screen-share tool",
  "desktop-workspace": "an adjacent desktop workspace organizer",
  "eng-tracker": "an engineering / Agile issue tracker",
  "docs-first": "a docs-first workspace with light project tracking",
  "spreadsheet-pmo": "a spreadsheet-style / PMO work management platform",
  "lightweight-board": "a lightweight board / Kanban project tool",
  "ai-calendar": "an AI calendar and auto-scheduling work tool",
  "docs-db-hybrid": "a flexible database / app builder with light project tracking",
};
...
export function buildReview(p) {
  ...
  const roleLabel = CLUSTER_LABELS[p.jobCluster] ?? "project management & productivity software";

  return {
    id: `review-${p.slug}-v1`,
    productSlug: p.slug,
    assessmentId: `assessment-${p.slug}-project-management-v1`,
    editorialStatus: "approved",
    title: `${p.name} Review (2026)`,
    h1: `${p.name} Review`,
    intro: `${p.name} is evaluated here as ${roleLabel} — ${p.shortDescription} This review uses SoftwareGlimpse’s project-management methodology (ease of use, work planning, automation, collaboration, integrations, reporting, scalability, value, AI), scoring products inside their job cluster rather than against unrelated peers. It is based on first-party research, not hands-on lab testing.`,
    ...
    factRefs: [
      ...
      {
        section: "features",
        factIds: PM_FEATURES.map((f) => `fact-${p.slug}-features.${f}`),
      },
    ],
    faq: [
      {
        question: `Is ${p.name} a project management / work OS tool?`,
        answer:
          p.jobCluster === "work-os"
            ? `Yes. ${p.name} is evaluated as a work OS / work management platform for boards, timelines, automations and team collaboration on work. It is not monday sales CRM — that remains a separate CRM product entity where applicable.`
            : p.jobCluster === "timeline"
              ? `Partially. ${p.name} is a specialist timeline / Gantt presentation tool — strong for executive timelines, not a full work OS peer to monday.com or Hive.`
              : p.jobCluster === "document-pdf"
                ? `No. ${p.name} is adjacent document / PDF productivity software. Compare it for PDF edit/convert/sign jobs — not as a work-OS peer on undifferentiated best ranks.`
                : p.jobCluster === "remote-access"
                  ? `No. ${p.name} is adjacent remote access / screen-share tooling. Pair it with a work OS if you also need boards, Gantt and workload planning.`
                  : `No. ${p.name} is an adjacent desktop workspace organizer. It wraps web apps into desktop shells — it is not a project tracker or work OS.`,
      },
      {
    ...
    seo: {
      title: `${p.name} Review (2026) — Project Management`,
      description: `${p.name} project management & productivity review on SoftwareGlimpse: strengths, trade-offs, pricing posture, and who should buy.`,
...
export function softSnippet(p) {
  ...
  return `  soft({
    ...
    primaryCategorySlug: "project-management",${secondary}
