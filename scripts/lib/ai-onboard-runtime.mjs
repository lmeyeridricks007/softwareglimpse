#!/usr/bin/env node
/**
 * Shared AI Software onboarding runtime.
 * Adapted from hr-onboard-runtime.mjs for ai-editorial v1.0.0.
 */
...
export let BATCH_LABEL = "AI Wave-1";
export let JOB_TAG = "ai-wave1";

export function configureAiRuntime(opts = {}) {
...
export const AI_CRITERION_WEIGHTS = {
  "ease-of-use": 12,
  "ai-job-fit": 16,
  "output-quality": 14,
  "workflow-depth": 12,
  integrations: 10,
  "governance-privacy": 10,
  scalability: 8,
  "value-for-money": 10,
  "model-capability": 8,
};
...
export function weightedScore(scores) {
  const total = AI_CRITERIA.reduce(
    (sum, criterion) => sum + scores[criterion] * AI_CRITERION_WEIGHTS[criterion],
    0,
  );
...
/** Flat plan billed monthly (e.g. GPU/credit subscriptions). */
...
export function buildProductFixture(p) {
  const featureLines = AI_FEATURES.map(
...
export function primaryJobFeature(jobCluster) {
  const map = {
    "llm-assistant": "llm-chat",
    "ai-writing": "writing-assist",
    "ai-voice": "voice-tts",
    "ai-presentations": "presentation-generation",
    "ai-website-builder": "website-generation",
    "ai-ad-creative": "ad-creative-generation",
    "ai-agents": "agent-builder",
  };
  return map[jobCluster] ?? "llm-chat";
}
...
export function supportingFeatureForCriterion(criterionSlug, jobCluster) {
  ...
  const map = {
    "ease-of-use": primary,
    "ai-job-fit": primary,
    "output-quality": "reasoning-models",
    "workflow-depth": "custom-projects",
    integrations: "connectors",
    "governance-privacy": "enterprise-admin",
    scalability: "usage-credits",
    "value-for-money": primary,
    "model-capability": "reasoning-models",
  };
...
export function buildEnrichment(p) {
  ...
  const featureSupport = AI_FEATURES.map((featureSlug) => ({
    featureSlug,
...
export function buildFacts(p) {
  ...
  for (const featureSlug of AI_FEATURES) {
...
export function buildCriterionAssessments(p) {
  return AI_CRITERIA.map((criterionSlug) => {
...
const OVERALL_RATIONALE =
  "Weighted average of the nine AI editorial criteria using category weights (AI job fit 16, output quality 14, ease of use 12, workflow depth 12, integrations 10, governance/privacy 10, value 10, model capability 8, scalability 8), rounded to 1 decimal. Products are scored inside their job cluster — LLM assistants, writing, voice, presentations, websites, ad creative, and agent builders are not forced into one undifferentiated ranking. Not a hands-on lab score, and affiliate economics are excluded.";
...
export function buildAssessment(p) {
  ...
  const adjacentBit =
    p.membershipRole === "adjacent"
      ? " Marked adjacent — not an LLM/writing/voice peer for undifferentiated AI best-page ranking."
      : "";
  return {
    id: `assessment-${p.slug}-ai-v1`,
    productSlug: p.slug,
    methodologySlug: "ai-editorial",
    methodologyVersion: "1.0.0",
    status: "approved",
    verdict: `${p.name}: ${p.whoShouldChoose}${adjacentBit} Scores use the AI editorial methodology from first-party research as of 2026-08-18 — not hands-on product testing.`,
    strengths: p.pros.slice(0, 5),
    weaknesses: p.cons.slice(0, 5),
    bestFor: p.bestFor,
    notIdealFor: p.notIdealFor,
    tradeoffs: [
      "Published free tiers vs paid usage/credit caps",
      "Bundled Workspace/One plans vs standalone assistant pricing",
      "Credit/overage models vs predictable seat pricing",
      "Enterprise admin controls vs individual consumer plans",
    ],
    recommendation: `${p.whoShouldChoose} ${p.whoShouldConsiderAlternatives}`,
    editorialNotes: `${BATCH_LABEL} batch. Approved on ai-editorial v1.0.0. membershipRole=${p.membershipRole}. jobCluster=${p.jobCluster}. handsOnTesting=false. Affiliate economics excluded.${
    ...
    scoreAudit: [
      {
        ...
        actor: "editorial",
        change:
          `${BATCH_LABEL} onboarding; approved AI criteria with category weights; handsOnTesting=false`,
...
const CLUSTER_LABELS = {
  "llm-assistant": "a general-purpose LLM assistant",
  "ai-writing": "an AI writing and paraphrasing assistant",
  "ai-voice": "an AI voice / text-to-speech platform",
  "ai-presentations": "an AI presentation and document generator",
  "ai-website-builder": "an AI website builder",
  "ai-ad-creative": "an AI ad creative generation platform",
  "ai-agents": "a no-code AI agent / app builder",
};
...
export function buildReview(p) {
  ...
  const roleLabel = CLUSTER_LABELS[p.jobCluster] ?? "AI software";

  return {
    id: `review-${p.slug}-v1`,
    productSlug: p.slug,
    assessmentId: `assessment-${p.slug}-ai-v1`,
    editorialStatus: "approved",
    title: `${p.name} Review (2026)`,
    h1: `${p.name} Review`,
    intro: `${p.name} is evaluated here as ${roleLabel} — ${p.shortDescription} This review uses SoftwareGlimpse’s AI methodology (ease of use, AI job fit, output quality, workflow depth, integrations, governance/privacy, scalability, value, model capability), scoring products inside their job cluster rather than against unrelated peers. It is based on first-party research, not hands-on lab testing.`,
    summary: p.whoShouldChoose,
    verdict: `${p.whoShouldChoose} ${p.whoShouldConsiderAlternatives} Scores reflect first-party documentation as of 2026-08-18 — not hands-on product testing. Confirm current packaging, usage caps and credit rates on the vendor site before purchase.`,
    overallScore,
    criterionAssessments,
    bestFor: p.bestFor,
    notIdealFor: p.notIdealFor,
    ...
    factRefs: [
      ...
      {
        section: "features",
        factIds: AI_FEATURES.map((f) => `fact-${p.slug}-features.${f}`),
      },
    ],
    faq: [
      {
        question: `Is ${p.name} AI software?`,
        answer: `Yes. ${p.name} is evaluated as ${CLUSTER_LABELS[p.jobCluster] ?? "AI software"}.`,
      },
      {
    ...
    seo: {
      title: `${p.name} Review (2026) — AI Software`,
      description: `${p.name} AI software review on SoftwareGlimpse: strengths, trade-offs, pricing posture, and who should buy.`,
...
export function softSnippet(p) {
  ...
  return `  soft({
    ...
    primaryCategorySlug: "ai",${secondary}
