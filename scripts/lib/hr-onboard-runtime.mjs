#!/usr/bin/env node
/**
 * Shared HR, Workforce & Training onboarding runtime.
 * Adapted from pm-onboard-runtime.mjs for hr-editorial v1.0.0.
 */
...
export let BATCH_LABEL = "HR Wave-1";
export let JOB_TAG = "hr-wave1";

export function configureHrRuntime(opts = {}) {
...
export const HR_CRITERION_WEIGHTS = {
  "ease-of-use": 12,
  "hiring-workforce-fit": 15,
  "workflow-depth": 14,
  integrations: 12,
  "mobile-frontline": 10,
  analytics: 8,
...
export function weightedScore(scores) {
  const total = HR_CRITERIA.reduce(
    (sum, criterion) => sum + scores[criterion] * HR_CRITERION_WEIGHTS[criterion],
    0,
  );
...
export function buildProductFixture(p) {
  const featureLines = HR_FEATURES.map(
...
export function primaryJobFeature(jobCluster) {
  const map = {
    "ats-recruiting": "applicant-tracking",
    "hris-core": "core-hris",
    "people-platform": "core-hris",
    "payroll-benefits": "payroll-processing",
    "enterprise-hcm": "core-hris",
    "frontline-wfm": "workforce-scheduling",
    "time-attendance": "time-attendance",
    "sop-training": "sop-knowledge-base",
    "lms-employee": "lms-course-commerce",
    "lms-marketing": "lms-course-commerce",
  };
  return map[jobCluster] ?? "applicant-tracking";
}
...
export function supportingFeatureForCriterion(criterionSlug, jobCluster) {
  ...
  const map = {
    "ease-of-use": primary,
    "hiring-workforce-fit": primary,
    "workflow-depth": primary,
    integrations: "hris-integrations",
    "mobile-frontline":
      jobCluster === "time-attendance" || jobCluster === "frontline-wfm"
        ? "gps-geofence-clockin"
...
export function buildEnrichment(p) {
  ...
  const featureSupport = HR_FEATURES.map((featureSlug) => ({
    featureSlug,
...
export function buildFacts(p) {
  ...
  for (const featureSlug of HR_FEATURES) {
...
export function buildCriterionAssessments(p) {
  return HR_CRITERIA.map((criterionSlug) => {
...
const OVERALL_RATIONALE =
  "Weighted average of the nine HR editorial criteria using the category weights (hiring/workforce fit 15, workflow depth 14, ease of use 12, integrations 12, value 12, mobile/frontline 10, scalability 9, analytics 8, AI 8), rounded to 1 decimal. Products are scored inside their job cluster — ATS, core HRIS, payroll/benefits, people-platform, enterprise HCM, frontline WFM, time & attendance, SOP training, and LMS peers are not forced into one undifferentiated ranking. Not a hands-on lab score, and affiliate economics are excluded.";
...
export function buildAssessment(p) {
  ...
  const adjacentBit =
    p.membershipRole === "adjacent"
      ? " Marked adjacent — not an ATS/WFM/time/SOP peer for undifferentiated HR best-page ranking."
      : "";
  return {
    id: `assessment-${p.slug}-hr-v1`,
    productSlug: p.slug,
    methodologySlug: "hr-editorial",
    methodologyVersion: "1.0.0",
    status: "approved",
    verdict: `${p.name}: ${p.whoShouldChoose}${adjacentBit} Scores use the HR editorial methodology from first-party research as of 2026-08-17 — not hands-on product testing.`,
    strengths: p.pros.slice(0, 5),
    weaknesses: p.cons.slice(0, 5),
    bestFor: p.bestFor,
    notIdealFor: p.notIdealFor,
    tradeoffs: [
      "Published free tiers vs paid depth for ATS, WFM, time, or training jobs",
      "Hub / multi-module pricing complexity vs single-product floors",
      "Mobile/frontline readiness vs desk-bound admin depth",
      "Opaque contact-sales pricing vs transparent published seats",
    ],
    recommendation: `${p.whoShouldChoose} ${p.whoShouldConsiderAlternatives}`,
    editorialNotes: `${BATCH_LABEL} batch. Approved on hr-editorial v1.0.0. membershipRole=${p.membershipRole}. jobCluster=${p.jobCluster}. handsOnTesting=false. Affiliate economics excluded.${
...
const CLUSTER_LABELS = {
  "ats-recruiting": "an applicant tracking / recruiting (ATS) platform",
  "hris-core": "a core HRIS / employee system of record",
  "people-platform": "a unified people platform (HR + payroll + IT modules)",
  "payroll-benefits": "a payroll and benefits administration platform",
  "enterprise-hcm": "an enterprise human capital management (HCM) suite",
  "frontline-wfm": "a frontline workforce management and scheduling platform",
  "time-attendance": "a time & attendance / clock-in platform",
  "sop-training": "an SOP / employee training documentation platform",
  "lms-employee": "an LMS / employee learning platform",
  "lms-marketing": "a marketing-primary LMS with secondary HR employee-learning fit",
};
...
export function buildReview(p) {
  ...
  return {
    id: `review-${p.slug}-v1`,
    productSlug: p.slug,
    assessmentId: `assessment-${p.slug}-hr-v1`,
    editorialStatus: "approved",
    title: `${p.name} Review (2026)`,
    h1: `${p.name} Review`,
    intro: `${p.name} is evaluated here as ${roleLabel} — ${p.shortDescription} This review uses SoftwareGlimpse’s HR methodology (ease of use, hiring/workforce fit, workflow depth, integrations, mobile/frontline, analytics, scalability, value, AI), scoring products inside their job cluster rather than against unrelated peers. It is based on first-party research, not hands-on lab testing.`,
    ...
    factRefs: [
      ...
      {
        section: "features",
        factIds: HR_FEATURES.map((f) => `fact-${p.slug}-features.${f}`),
      },
    ],
    faq: [
      {
        question: `Is ${p.name} HR / workforce software?`,
        answer: `Yes. ${p.name} is evaluated as ${CLUSTER_LABELS[p.jobCluster] ?? "HR, workforce & training software"}.`,
      },
      {
    ...
    seo: {
      title: `${p.name} Review (2026) — HR & Workforce`,
...
export function softSnippet(p) {
  ...
  return `  soft({
    ...
    primaryCategorySlug: "hr",${secondary}
