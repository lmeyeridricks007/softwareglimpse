#!/usr/bin/env node
/**
 * Shared Customer Service onboarding runtime.
 * Adapted from hr-onboard-runtime.mjs for customer-service-editorial v1.0.0.
 */
...
export let BATCH_LABEL = "CS Wave-1";
export let JOB_TAG = "cs-wave1";

export function configureCsRuntime(opts = {}) {
...
export const CS_CRITERION_WEIGHTS = {
  "ease-of-use": 12,
  "support-job-fit": 15,
  "workflow-depth": 12,
  omnichannel: 10,
...
/** Per-seat plan on monthly billing — amount is monthly. */
...
export function weightedScore(scores) {
  const total = CS_CRITERIA.reduce(
    (sum, criterion) => sum + scores[criterion] * CS_CRITERION_WEIGHTS[criterion],
    0,
  );
...
export function buildProductFixture(p) {
  const featureLines = CS_FEATURES.map(
    (f) => `FEATURE ${f}: ${featureAvailability(p, f)}`,
  ).join("\n");
  const ai = (p.aiLines ?? ["AI agent: unknown"]).join("\n");
  const adjacent =
...
export function primaryJobFeature(jobCluster) {
  const map = {
    "helpdesk-ticketing": "ticketing",
    "live-chat-support": "live-chat",
    "ecommerce-helpdesk": "ecommerce-helpdesk",
    "itsm-service-desk": "itsm-service-desk",
  };
  return map[jobCluster] ?? "ticketing";
}
...
export function supportingFeatureForCriterion(criterionSlug, jobCluster) {
  ...
  const map = {
    ...
    "support-job-fit": primary,
    "workflow-depth": "ticketing",
    omnichannel: "omnichannel-inbox",
    "self-service": "knowledge-base",
    integrations: "helpdesk-integrations",
    analytics: "helpdesk-reporting",
    scalability: primary,
    "value-for-money": primary,
    "ai-capabilities": "chatbot-ai-agent",
  };
...
export function buildEnrichment(p) {
  ...
  const featureSupport = CS_FEATURES.map((featureSlug) => ({
    featureSlug,
    ...
    const strength =
      p.membershipRole === "adjacent"
        ...
        : businessSizeSlug === "enterprise"
          ? p.jobCluster === "helpdesk-ticketing"
            ? "strong"
            : "moderate"
          : "strong";
    return {
      ...
      rationale: `${p.name} fit for ${businessSizeSlug} ${primaryTeamType} teams in the ${p.jobCluster} job cluster, from first-party positioning and ${BATCH_LABEL} research${
        p.membershipRole === "adjacent"
          ? " (adjacent tool — not a CS peer for undifferentiated ranking)"
          : ""
      }.`,
      isEditorial: true,
    };
...
export function buildFacts(p) {
  ...
  for (const featureSlug of CS_FEATURES) {
...
export function buildCriterionAssessments(p) {
  return CS_CRITERIA.map((criterionSlug) => {
...
const OVERALL_RATIONALE =
  "Weighted average of the ten customer-service editorial criteria using category weights (support job fit 15, ease of use 12, workflow depth 12, omnichannel 10, integrations 10, value 10, self-service 8, analytics 8, scalability 8, AI 7), rounded to 1 decimal. Products are scored inside their job cluster — helpdesk, live chat, ecommerce helpdesk, and ITSM peers are not forced into one undifferentiated ranking. Not a hands-on lab score, and affiliate economics are excluded.";
...
export function buildAssessment(p) {
  ...
  const adjacentBit =
    p.membershipRole === "adjacent"
      ? " Marked adjacent — not a helpdesk/live-chat/ITSM peer for undifferentiated CS best-page ranking."
      : "";
  return {
    id: `assessment-${p.slug}-cs-v1`,
    productSlug: p.slug,
    methodologySlug: "customer-service-editorial",
    methodologyVersion: "1.0.0",
    status: "approved",
    verdict: `${p.name}: ${p.whoShouldChoose}${adjacentBit} Scores use the customer-service editorial methodology from first-party research as of 2026-08-18 — not hands-on product testing.`,
    strengths: p.pros.slice(0, 5),
    weaknesses: p.cons.slice(0, 5),
    bestFor: p.bestFor,
    notIdealFor: p.notIdealFor,
    tradeoffs: [
      "Per-agent seats vs ticket/conversation caps and AI outcome pricing",
      "Omnichannel breadth vs SMB simplicity in shared inbox tools",
      "Suite upsell complexity vs transparent published plan floors",
      "ITSM depth vs lightweight live-chat or ecommerce helpdesk fit",
    ],
    recommendation: `${p.whoShouldChoose} ${p.whoShouldConsiderAlternatives}`,
    editorialNotes: `${BATCH_LABEL} batch. Approved on customer-service-editorial v1.0.0. membershipRole=${p.membershipRole}. jobCluster=${p.jobCluster}. handsOnTesting=false. Affiliate economics excluded.${
    ...
    scoreAudit: [
      {
        ...
        actor: "editorial",
        change:
          `${BATCH_LABEL} onboarding; approved CS criteria with category weights; handsOnTesting=false`,
...
const CLUSTER_LABELS = {
  "helpdesk-ticketing": "a helpdesk / ticketing platform",
  "live-chat-support": "a live chat and website messenger support platform",
  "ecommerce-helpdesk": "an ecommerce helpdesk with order-aware support workflows",
  "itsm-service-desk": "an IT service desk / ITSM platform",
};
...
export function buildReview(p) {
  ...
  const roleLabel = CLUSTER_LABELS[p.jobCluster] ?? "customer service software";

  return {
    id: `review-${p.slug}-v1`,
    productSlug: p.slug,
    assessmentId: `assessment-${p.slug}-cs-v1`,
    editorialStatus: "approved",
    title: `${p.name} Review (2026)`,
    h1: `${p.name} Review`,
    intro: `${p.name} is evaluated here as ${roleLabel} — ${p.shortDescription} This review uses SoftwareGlimpse's customer-service methodology (ease of use, support job fit, workflow depth, omnichannel, self-service, integrations, analytics, scalability, value, AI), scoring products inside their job cluster rather than against unrelated peers. It is based on first-party research, not hands-on lab testing.`,
    summary: p.whoShouldChoose,
    verdict: `${p.whoShouldChoose} ${p.whoShouldConsiderAlternatives} Scores reflect first-party documentation as of 2026-08-18 — not hands-on product testing. Confirm current packaging, minimums and usage rates on the vendor site before purchase.`,
    overallScore,
    criterionAssessments,
    bestFor: p.bestFor,
    notIdealFor: p.notIdealFor,
    ...
    factRefs: [
      ...
      {
        section: "features",
        factIds: CS_FEATURES.map((f) => `fact-${p.slug}-features.${f}`),
      },
    ],
    faq: [
      {
        question: `Is ${p.name} customer service software?`,
        answer: `Yes. ${p.name} is evaluated as ${CLUSTER_LABELS[p.jobCluster] ?? "customer service software"}.`,
      },
      {
    ...
    seo: {
      title: `${p.name} Review (2026) — Customer Service`,
      description: `${p.name} customer service review on SoftwareGlimpse: strengths, trade-offs, pricing posture, and who should buy.`,
