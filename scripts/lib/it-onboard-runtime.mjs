#!/usr/bin/env node
/**
 * Shared IT & Development onboarding runtime.
 * Adapted from hr-onboard-runtime.mjs for it-development-editorial v1.0.0.
 */
...
export let VERIFIED_AT = "2026-08-18T12:00:00.000Z";
export let PUBLISHED_AT = "2026-08-18T00:00:00.000Z";
export let RELATED_GUIDE_PATHS = [
  "/categories/it-development/",
  "/best/it-development-software/",
];
export let BATCH_LABEL = "IT Wave-1";
export let JOB_TAG = "it-wave1";

export function configureItRuntime(opts = {}) {
...
export const IT_CRITERION_WEIGHTS = {
  "ease-of-use": 12,
  "it-job-fit": 16,
  "workflow-depth": 14,
  integrations: 12,
...
/** Flat plan billed monthly (e.g. hosting panel licences). */
export function planFlatMonthly(slug, name, monthly, extra = {}) {
  return {
    id: `plan-${slug}`,
    slug,
    name,
...
/** Per-host plan quoted as monthly-equivalent on annual billing (observability). */
export function planPerHostAnnual(slug, name, monthlyPerHost, extra = {}) {
  return {
    id: `plan-${slug}`,
    slug,
    name,
    ...
    rules: [
      {
        kind: "usage",
        amountPerUnit: monthlyPerHost,
        unit: "host",
        currency: "USD",
        interval: "year",
        amountPeriod: "month",
        ...(extra.minimumUnits ? { minimumUnits: extra.minimumUnits } : {}),
      },
    ],
    ...(extra.description ? { description: extra.description } : {}),
    ...(extra.limits ? { limits: extra.limits } : {}),
...
export function weightedScore(scores) {
  const total = IT_CRITERIA.reduce(
    (sum, criterion) => sum + scores[criterion] * IT_CRITERION_WEIGHTS[criterion],
    0,
  );
  ...
  const sources = [
    {
      ...
      confidence: "high",
      status: "active",
      notes: `First-party product positioning for ${p.name} (${BATCH_LABEL} onboarding 2026-08-18).`,
    },
    {
...
export function buildPricingFixture(p) {
  return `# FIXTURE SNAPSHOT — structured from first-party research (2026-08-18)
...
export function buildProductFixture(p) {
  const featureLines = IT_FEATURES.map(
...
export function primaryJobFeature(jobCluster) {
  const map = {
    ...
    "observability-monitoring": "infrastructure-monitoring",
    "source-control-devops": "source-control",
    "hosting-operations": "hosting-panel",
    "hosting-providers": "managed-hosting",
    "web-data-collection": "proxy-network",
    "cloud-paas": "cloud-paas",
  };
  return map[jobCluster] ?? "incident-management";
}

export function workflowDepthFeature(jobCluster) {
  const map = {
    "itsm-service-desk": "change-problem",
    "observability-monitoring": "apm-tracing",
    "source-control-devops": "cicd-actions",
    "hosting-operations": "hosting-panel",
    "hosting-providers": "managed-hosting",
    "web-data-collection": "proxy-network",
    "cloud-paas": "cicd-actions",
  };
  return map[jobCluster] ?? primaryJobFeature(jobCluster);
}

export function aiFeatureForCluster(jobCluster) {
  if (jobCluster === "itsm-service-desk") return "itsm-ai";
  if (jobCluster === "source-control-devops") return "dev-ai";
  if (jobCluster === "cloud-paas") return "dev-ai";
  return "itsm-ai";
}
...
export function supportingFeatureForCriterion(criterionSlug, jobCluster) {
  ...
  const map = {
    "ease-of-use": primary,
    "it-job-fit": primary,
    "workflow-depth": workflowDepthFeature(jobCluster),
    integrations: "analytics-reporting",
    "admin-security": "enterprise-security",
    scalability: "enterprise-security",
    "value-for-money": primary,
    "ai-capabilities": aiFeatureForCluster(jobCluster),
  };
...
export function buildEnrichment(p) {
  ...
  const featureSupport = IT_FEATURES.map((featureSlug) => ({
    featureSlug,
  ...
  const editorialFit = (p.businessSizeSlugs ?? []).map((businessSizeSlug) => {
    ...
    return {
      ...
      rationale: `${p.name} fit for ${businessSizeSlug} ${primaryTeamType} teams in the ${p.jobCluster} job cluster, from first-party positioning and ${BATCH_LABEL} research${
        p.membershipRole === "adjacent"
          ? " (adjacent tool — not an IT peer for undifferentiated ranking)"
          : ""
      }.`,
      isEditorial: true,
    };
...
export function buildFacts(p) {
  ...
  for (const featureSlug of IT_FEATURES) {
...
export function buildCriterionAssessments(p) {
  return IT_CRITERIA.map((criterionSlug) => {
...
const OVERALL_RATIONALE =
  "Weighted average of the eight IT editorial criteria using category weights (IT job fit 16, workflow depth 14, ease of use 12, integrations 12, admin/security 12, value 12, AI 12, scalability 10), rounded to 1 decimal. Products are scored inside their job cluster — ITSM, observability, source control, hosting panels, and web-data platforms are not forced into one undifferentiated ranking. Not a hands-on lab score, and affiliate economics are excluded.";
...
export function buildAssessment(p) {
  ...
  const adjacentBit =
    p.membershipRole === "adjacent"
      ? " Marked adjacent — not an ITSM/observability/source-control peer for undifferentiated IT best-page ranking."
      : "";
  return {
    id: `assessment-${p.slug}-it-v1`,
    productSlug: p.slug,
    methodologySlug: "it-development-editorial",
    methodologyVersion: "1.0.0",
    status: "approved",
    verdict: `${p.name}: ${p.whoShouldChoose}${adjacentBit} Scores use the IT & Development editorial methodology from first-party research as of 2026-08-18 — not hands-on product testing.`,
    strengths: p.pros.slice(0, 5),
    weaknesses: p.cons.slice(0, 5),
    bestFor: p.bestFor,
    notIdealFor: p.notIdealFor,
    tradeoffs: [
      "Per-agent vs per-host vs per-seat vs per-GB pricing models",
      "Module stacking and consumption overages vs single-SKU floors",
      "Free tiers vs enterprise SSO/audit gates",
      "Landscape-adjacent tools vs primary job-cluster peers on best pages",
    ],
    recommendation: `${p.whoShouldChoose} ${p.whoShouldConsiderAlternatives}`,
    editorialNotes: `${BATCH_LABEL} batch. Approved on it-development-editorial v1.0.0. membershipRole=${p.membershipRole}. jobCluster=${p.jobCluster}. handsOnTesting=false. Affiliate economics excluded.${
    ...
    scoreAudit: [
      {
        ...
        actor: "editorial",
        change:
          `${BATCH_LABEL} onboarding; approved IT criteria with category weights; handsOnTesting=false`,
...
const CLUSTER_LABELS = {
  "itsm-service-desk": "an IT service management (ITSM) / employee service desk platform",
  "observability-monitoring": "an observability and infrastructure monitoring suite",
  "source-control-devops": "a source control and DevOps collaboration platform",
  "hosting-operations": "a hosting / server control panel for web operations",
  "web-data-collection": "a web data / proxy collection platform",
};
...
export function buildReview(p) {
  ...
  const roleLabel = CLUSTER_LABELS[p.jobCluster] ?? "IT & development software";

  return {
    id: `review-${p.slug}-v1`,
    productSlug: p.slug,
    assessmentId: `assessment-${p.slug}-it-v1`,
    editorialStatus: "approved",
    title: `${p.name} Review (2026)`,
    h1: `${p.name} Review`,
    intro: `${p.name} is evaluated here as ${roleLabel} — ${p.shortDescription} This review uses SoftwareGlimpse’s IT & Development methodology (ease of use, IT job fit, workflow depth, integrations, admin/security, scalability, value, AI), scoring products inside their job cluster rather than against unrelated peers. It is based on first-party research, not hands-on lab testing.`,
    summary: p.whoShouldChoose,
    verdict: `${p.whoShouldChoose} ${p.whoShouldConsiderAlternatives} Scores reflect first-party documentation as of 2026-08-18 — not hands-on product testing. Confirm current packaging, host/GB rates and minimums on the vendor site before purchase.`,
    overallScore,
    criterionAssessments,
    bestFor: p.bestFor,
    notIdealFor: p.notIdealFor,
    ...
    factRefs: [
      ...
      {
        section: "features",
        factIds: IT_FEATURES.map((f) => `fact-${p.slug}-features.${f}`),
      },
    ],
    faq: [
      {
        question: `Is ${p.name} IT / development software?`,
        answer: `Yes. ${p.name} is evaluated as ${CLUSTER_LABELS[p.jobCluster] ?? "IT & development software"}.`,
      },
      {
    ...
    seo: {
      title: `${p.name} Review (2026) — IT & Development`,
      description: `${p.name} IT & development review on SoftwareGlimpse: strengths, trade-offs, pricing posture, and who should buy.`,
...
export function softSnippet(p) {
  ...
  return `  soft({
    ...
    primaryCategorySlug: "it-development",${secondary}
