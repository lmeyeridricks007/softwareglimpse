#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_bc-priority1-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-bc-priority1-batch.mjs
 *   node scripts/patch-software-seed-bc-priority1.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_bc-priority1-seed-snippet.ts");

/** Wave-1 left ~97; refuse if seed looks truncated. */
const MIN_EXPECTED_ENTRIES = 97;

const require = createRequire(import.meta.url);
// Reuse wave-1 patcher helpers by inlining the same logic (keep standalone).
...
function main() {
  ...
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-bc-priority1-batch.mjs`,
    );
    process.exit(1);
  }
