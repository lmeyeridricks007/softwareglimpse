#!/usr/bin/env node
/**
 * Append IT industry Tier-A comparison pairs into comparisons.ts
 * before comparisonsSeedAuthored.
 *
 * Same-cluster first; kinsta vs plesk and sentry vs pagerduty are landscape.
 */
...
function buildPair(s, landscape) {
  ...
  return `  approvedItPair({
    ...
    factual: {
      startingPricing: "Starting floors: ${titleA} ~$${spA} vs ${titleB} ~$${spB} — confirm live packaging (host, seat, GB, CI-minute, and panel vs managed-host math differ).",
      freePlan: "Compare published free tiers, trials, and first-month promotions on each vendor pricing page.",
      userMinimum: "Check annual vs monthly billing, add-ons, and whether the SKU is observability, on-call, CI, panel, managed hosting, or proxy GB.",
    },
...
const itAll = bySlug([...IT_W1, ...IT_P2, ...HOSTING, ...TIERA]);

const pairs = [
  ["splunk", "datadog", false],
  ["elastic-observability", "grafana-cloud", false],
  ["sentry", "datadog", false],
  ["incident-io", "pagerduty", false],
  ["circleci", "github", false],
  ["directadmin", "plesk", false],
  ["kinsta", "wp-engine", false],
  ["smartproxy", "bright-data", false],
  ["splunk", "elastic-observability", false],
  ["sentry", "splunk", false],
  ["kinsta", "plesk", true],
  ["sentry", "pagerduty", true],
];
...
fs.writeFileSync(
  path.join(ROOT, "scripts/_it-industry-tiera-comparisons.json"),
  `${JSON.stringify(specs, null, 2)}\n`,
);

let src = fs.readFileSync(TARGET, "utf8");
if (src.includes('a: "splunk"') || src.includes("splunk-vs-datadog")) {
  console.log("IT industry Tier-A comparisons already present — skip append");
  process.exit(0);
}
