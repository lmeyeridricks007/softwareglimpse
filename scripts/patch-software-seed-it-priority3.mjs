#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_it-priority3-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-it-priority3-batch.mjs
 *   node scripts/patch-software-seed-it-priority3.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_it-priority3-seed-snippet.ts");
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-it-priority2-batch.mjs`,
    );
    process.exit(1);
  }
