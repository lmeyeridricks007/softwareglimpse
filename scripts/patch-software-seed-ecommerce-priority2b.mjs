#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_ecommerce-priority2b-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-ecommerce-priority2b-batch.mjs
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_ecommerce-priority2b-seed-snippet.ts");
...
function findSoftwareSeedClose(source) {
  const rawIdx = source.indexOf("const softwareSeedRaw");
  if (rawIdx < 0) {
    throw new Error("Could not find `const softwareSeedRaw` in software.ts");
  }
  const exportIdx = source.indexOf("\nexport const softwareSeed", rawIdx);
  if (exportIdx < 0) {
    throw new Error("Could not find `export const softwareSeed` after softwareSeedRaw");
  }
  const before = source.slice(0, exportIdx);
  const closeIdx = before.lastIndexOf("];");
  if (closeIdx < 0) {
    throw new Error("Could not find closing `];` of softwareSeedRaw");
  }
  return closeIdx;
}
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-ecommerce-priority2b-batch.mjs`,
    );
    process.exit(1);
  }
