#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_ai-automation-overlay-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-ai-automation-overlay-batch.mjs
 *   node scripts/patch-software-seed-ai-automation-overlay.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_ai-automation-overlay-seed-snippet.ts");

/** Prior batches left ~226+; refuse if seed looks truncated. */
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-ai-automation-overlay-batch.mjs`,
    );
    process.exit(1);
  }
