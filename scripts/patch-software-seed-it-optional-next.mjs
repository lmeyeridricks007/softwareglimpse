#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_it-optional-next-seed-snippet.ts into
 * src/data/seed/software.ts before `export const softwareSeed`
 * (closing `];` of softwareSeedRaw).
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-it-optional-next-batch.mjs
 *   node scripts/patch-software-seed-it-optional-next.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_it-optional-next-seed-snippet.ts");

/** Prior IT batches left 240+; refuse if seed looks truncated. */
const MIN_EXPECTED_ENTRIES = 240;
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-it-optional-next-batch.mjs`,
    );
    process.exit(1);
  }
