#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_ai-priority2-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-ai-priority2-batch.mjs
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_ai-priority2-seed-snippet.ts");

/** Wave-1 left ~198; refuse if seed looks truncated. */
const MIN_EXPECTED_ENTRIES = 198;
