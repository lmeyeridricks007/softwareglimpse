#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_it-webdata-peers-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-it-webdata-peers-batch.mjs
 *   node scripts/patch-software-seed-it-webdata-peers.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_it-webdata-peers-seed-snippet.ts");

/** Priority-2 left 219; refuse if seed looks truncated. */
const MIN_EXPECTED_ENTRIES = 226;
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-it-webdata-peers-batch.mjs`,
    );
    process.exit(1);
  }
