#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_ai-wave1-seed-snippet.ts into
 ...
 *   node scripts/onboard-ai-wave1-batch.mjs
 *   node scripts/patch-software-seed-ai-wave1.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_ai-wave1-seed-snippet.ts");
...
function main() {
  ...
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-ai-wave1-batch.mjs`,
    );
    process.exit(1);
  }
