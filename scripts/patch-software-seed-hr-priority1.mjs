#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_hr-priority1-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-hr-priority1-batch.mjs
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_hr-priority1-seed-snippet.ts");

/** Wave-1 left ~158; refuse if seed looks truncated. */
const MIN_EXPECTED_ENTRIES = 155;
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-hr-priority1-batch.mjs`,
    );
    process.exit(1);
  }
