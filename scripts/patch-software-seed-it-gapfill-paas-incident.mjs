#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_it-gapfill-paas-incident-seed-snippet.ts into
 * src/data/seed/software.ts before `export const softwareSeed`
 * (closing `];` of softwareSeedRaw).
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-it-gapfill-paas-incident-batch.mjs
 *   node scripts/patch-software-seed-it-gapfill-paas-incident.mjs
 */
...
const SNIPPET_PATH = path.join(
  ROOT,
  "scripts/_it-gapfill-paas-incident-seed-snippet.ts",
);

/** Prior IT batches left 270+; refuse if seed looks truncated. */
const MIN_EXPECTED_ENTRIES = 270;
...
function findSoftwareSeedClose(source) {
  ...
  if (fnIdx < 0) {
    throw new Error("Could not find softwareSeed export or `function soft(`");
  }
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-it-gapfill-paas-incident-batch.mjs`,
    );
    process.exit(1);
  }
