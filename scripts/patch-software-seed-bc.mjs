#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_bc-wave1-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed
 * (the ]; that appears before `function soft`).
 *
 * Append-only and idempotent: existing soft-{slug} / slug entries are skipped,
 * so a concurrent agent's products are never rewritten or removed.
 *
 * Usage:
 *   node scripts/onboard-bc-wave1-batch.mjs
 *   node scripts/patch-software-seed-bc.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_bc-wave1-seed-snippet.ts");

/** Minimum seed entries expected before patching (corruption guard). */
const MIN_EXPECTED_ENTRIES = 89;
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-bc-wave1-batch.mjs`,
    );
    process.exit(1);
  }
  ...
  if (existing < MIN_EXPECTED_ENTRIES) {
    console.error(
      `Refusing to patch: software.ts has ${existing} soft() entries, expected at least ${MIN_EXPECTED_ENTRIES}. Restore the seed before appending.`,
    );
    process.exit(1);
  }
  ...
  const after = countExistingEntries(next);
