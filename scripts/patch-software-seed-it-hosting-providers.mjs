#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_it-hosting-providers-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-it-hosting-providers-batch.mjs
 *   node scripts/patch-software-seed-it-hosting-providers.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_it-hosting-providers-seed-snippet.ts");
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-it-hosting-providers-batch.mjs`,
    );
    process.exit(1);
  }
