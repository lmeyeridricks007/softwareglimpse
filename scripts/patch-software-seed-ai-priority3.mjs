#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_ai-priority3-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-ai-priority3-batch.mjs
 *   node scripts/patch-software-seed-ai-priority3.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_ai-priority3-seed-snippet.ts");

/** Priority-2 left 219; refuse if seed looks truncated. */
const MIN_EXPECTED_ENTRIES = 219;
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-ai-priority2-batch.mjs`,
    );
    process.exit(1);
  }
