#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_hr-priority3-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-hr-priority3-batch.mjs
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_hr-priority3-seed-snippet.ts");

/** Priority-2 left 171; refuse if seed looks truncated. */
const MIN_EXPECTED_ENTRIES = 171;
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-hr-priority3-batch.mjs`,
    );
    process.exit(1);
  }
