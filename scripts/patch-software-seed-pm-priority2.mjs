#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_pm-priority2-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-pm-priority2-batch.mjs
 *   node scripts/patch-software-seed-pm-priority2.mjs
 */
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_pm-priority2-seed-snippet.ts");

/** Prior batches left ~115; refuse if seed looks truncated. */
...
function main() {
  ...
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-pm-priority2-batch.mjs`,
    );
    process.exit(1);
  }
