#!/usr/bin/env node
/**
 * Appends soft() entries from scripts/_hr-priority2-seed-snippet.ts into
 * src/data/seed/software.ts before the closing ]; of softwareSeed.
 *
 * Append-only and idempotent. Usage:
 *   node scripts/onboard-hr-priority2-batch.mjs
...
const SNIPPET_PATH = path.join(ROOT, "scripts/_hr-priority2-seed-snippet.ts");

/** Priority-1 left 163; refuse if seed looks truncated. */
const MIN_EXPECTED_ENTRIES = 163;
...
function main() {
  if (!fs.existsSync(SNIPPET_PATH)) {
    console.error(
      `Missing ${SNIPPET_PATH}. Run: node scripts/onboard-hr-priority2-batch.mjs`,
    );
    process.exit(1);
  }
